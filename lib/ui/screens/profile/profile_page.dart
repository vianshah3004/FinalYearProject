...
import '../auth/EnableBiometricPage.dart';
...
import '../settings/license_agreement.dart';
import '../settings/notifications/NotificationsPage.dart';
import '../support/about_us.dart';
...
class _FeedbackPageState extends State<FeedbackPage> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final padding = size.width * 0.05; // Responsive padding (5% of screen width)
    final dialogWidth = size.width * 0.85; // Dialog takes 85% of screen width
    final fontSizeTitle = (size.width * 0.045).clamp(14.0, 18.0); // Clamped title font size
    final fontSizeText = (size.width * 0.04).clamp(12.0, 16.0); // Clamped text font size
    final buttonPadding = size.width * 0.08; // Responsive button padding
                ...
                child: Dialog(
                  ...
                  child: Container(
                    ...
                    constraints: BoxConstraints(
                      maxHeight: size.height * 0.7, // Limit dialog height
                    ),
                    child: SingleChildScrollView(
                      child: Padding(
                        ...
                        child: _submitted
                          ...
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Text("😊", style: TextStyle(fontSize: (size.width * 0.12) * 1.5)),
                          ...
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            // Row for "Give Feedback" and close button
                            LayoutBuilder(
                              builder: (context, constraints) {
                                final rowWidth = constraints.maxWidth;
                                final iconSize = (rowWidth * 0.1).clamp(16.0, 20.0);
                                final textWidth = rowWidth * 0.7;
                            ...
                            // Modified EmojiFeedback to prevent overflow
                            LayoutBuilder(
                              builder: (context, constraints) {
                                final availableWidth = constraints.maxWidth; // 265.6 pixels
                                // Assume 5 emojis, account for potential internal spacing
                                final emojiCount = 5; // Adjust if different
                                // Reduce elementSize to account for 1.6-pixel overflow
                                final maxElementSize = (availableWidth / emojiCount * 0.9).clamp(25.0, 40.0);
                                return ClipRect(
                                  child: SingleChildScrollView(
                            ...
                            if (_showWarning)
                              Padding(
                                ...
                                child: Row(
                                  children: [
                                    Icon(Icons.warning,
                                        color: Colors.red, size: fontSizeText),
